...
- @Render responsive players@ :=
*Default:* no
*valid:* yes or no =:
- @Add microdata@ :=
Add @itemscope@ and the right @itemtype@ to the player wraptag (uses a @div@ if not provided) and add a @meta@ tag declaring the @embedURL@ property right after the embedding code.
*Default:* no
*valid:* yes or no =:
